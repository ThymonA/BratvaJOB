...
function OpenWeaponSafeMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'weapon_safe',
        {
...
function OpenWeaponBuyMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'buy_weapons',
        {
...
function BuyWeaponConfirm(weapon_name, weapon_price)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'buy_weapon',
        {
    ...
    ESX.TriggerServerCallback('ml_' .. Config.JobName .. 'job:getWeapon', function(weapons)
        for i = 1, #weapons, 1 do
            ...
            if (weapon ~= nil and weapon.count ~= nil and weapon.count > 0) then
                ...
                table.insert(elements, { label = _U('safe_label', weapon_label, weapon.count), value = weapon.name })
            end
        end

        ESX.UI.Menu.Open(
            'default', GetCurrentResourceName(), 'stored_weapons',
            {
                title    = _U('weapon_safe'),
                align    = 'top-left',
                css      = Config.JobName,
...
function OpenWeaponDeposit()
    ...
    ESX.TriggerServerCallback('ml_' .. Config.JobName .. 'job:getPlayerWeapons', function(weapons)
        ...
        ESX.UI.Menu.Open(
            'default', GetCurrentResourceName(), 'store_own_weapons',
            {
